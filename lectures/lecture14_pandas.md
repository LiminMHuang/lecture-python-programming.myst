...
Here is a popularity comparison over time against Matlab and STATA courtesy of (经由...提供) Stack Overflow Trends
...
1. endows them with methods that facilitate operations such as
  ...
  - sorting, grouping, re-ordering and general data munging (处理（计算机数据）) <sup><a href=#mung id=mung-link>[1]</a></sup>
  - dealing with missing values, etc., etc.


More sophisticated statistical functionality is left to other packages, such as [statsmodels](https://www.statsmodels.org/) and [scikit-learn](https://scikit-learn.org/), which are built on top of pandas.
...
In essence, a `DataFrame` in pandas is analogous to a (highly optimised) Excel spreadsheet.

Thus, it is a powerful tool for representing and analysing data that are naturally organised into rows and columns, often with descriptive indexes for individual rows and individual columns.

Let us look at an example that reads data from the CSV file `pandas/data/test_pwt.csv`, which is taken from the [Penn World Tables](https://www.rug.nl/ggdc/productivity/pwt/pwt-releases/pwt-7.0).
...
We will read this in from a URL using the `pandas` function `read_csv`.
...
Let us imagine that we are only interested in the population (`POP`) and total GDP (`tcgdp`).
...
Let us give the columns slightly better names
...
The `population` variable is in thousands, let us revert to single units
...
Next, we are going to add a column showing real GDP per capita, multiplying by 1,000,000 as we go because total GDP is in millions
...
One of the nice things about pandas `DataFrame` and `Series` objects is that they have methods for plotting and visualisation that work through Matplotlib.
...
At the moment the data frame is ordered alphabetically on the countries — let us change it to GDP per capita
...
Alternatively, we can access the CSV file from within a Python programme.
...
If there is no error message, then the call has succeeded.

If you do get an error, then there are two likely causes

1. You are not connected to the Internet — hopefully, this is not the case.
1. Your machine is accessing the Internet through a proxy server, and Python is not aware of this.
...
We could now write some additional code to parse (（计算机）句法分析) this text and store it as an array.

But this is unnecessary — pandas’ `read_csv` function can handle the task for us.

We use `parse_dates=True` so that pandas recognises our dates column, allowing for simple date filtering
...
For now let us work through one example of downloading and plotting data — this
time from the World Bank.

The World Bank [collects and organises data](https://data.worldbank.org/indicator) on a huge range of indicators.

For example, [here is](https://data.worldbank.org/indicator/GC.DOD.TOTL.GD.ZS) some data on government debt as a ratio to GDP.
